"""
bt_analysis/ の JSON群から各口座のHTMLを生成する
使い方: python3 build_html.py [A|B|C|all]
引数なしまたは all → 全口座を生成
"""
import json, sys
...
# 口座別ペア定義（出力ファイル名・タイトル・対象JSONプレフィックス）
ACCOUNT_CONFIG = {
    'A': {
        'out':   BASE / 'account_a.html',
        'title': '口座A',
        'pairs': ['EURUSD', 'EURGBP', 'USDCHF'],
    },
    'B': {
        'out':   BASE / 'account_b.html',
        'title': '口座B',
        'pairs': ['USDJPY', 'GBPUSD', 'AUDJPY'],
    },
    'C': {
        'out':   BASE / 'account_c.html',
        'title': '口座C',
        'pairs': ['AUDUSD', 'EURJPY'],
    },
}
...
# どの口座を生成するか
args = sys.argv[1:]
targets = list(ACCOUNT_CONFIG.keys()) if not args or args[0].upper() == 'ALL' else [a.upper() for a in args]
...
PAIR_JPY_PER_LOT_PER_PIP = {
    'EURUSD': PIP_SIZE * CONTRACT * 150,         # USD建て → USDJPY=150
    'EURGBP': PIP_SIZE * CONTRACT * 190,         # GBP建て → GBPJPY≈190
    'USDCHF': PIP_SIZE * CONTRACT * 150 / 0.90,  # CHF建て → CHFJPY≈167
    'USDJPY': PIP_SIZE * CONTRACT * 1,           # JPY建て → 1pip=1000円/lot (pip=0.01)
    'GBPUSD': PIP_SIZE * CONTRACT * 150,         # USD建て → USDJPY=150
    'AUDJPY': PIP_SIZE * CONTRACT * 1,           # JPY建て
    'AUDUSD': PIP_SIZE * CONTRACT * 150,         # USD建て → USDJPY=150
    'EURJPY': PIP_SIZE * CONTRACT * 1,           # JPY建て
}
RATE_NOTE = 'USDJPY=150 / GBPJPY=190 / CHFJPY=167 / JPY系は1pip=直接円（固定近似）'
...
for ACCOUNT in targets:
    ...
    <title>PEACE BT Analysis — {ACCT_TITLE}</title>
    <style>
    *{{box-sizing:border-box;margin:0;padding:0}}
    ...
    <header>
      ...
      <span>{ACCT_TITLE} — 2023〜2025 / HTM→JSON変換済み</span>
    </header>
    <main>
